...
import { SQSListenerEvent } from './interfaces/SQSListenerEvent';
import { JSONSerializer } from './JSONSerializer';
import { SQSSerializer } from './interfaces/SQSSerializer';
import { SQSRoute } from './interfaces/SQSRoute';
...
export abstract class SQSRouter {
  private static logger: Logger = LoggerService.named({ name: 'SQSRouter', adornments: { '@mu-ts': 'sqs' } });
   ...
   */
  public static async handle(_event: SQSEvent, _context: Context, callback: Callback) {
    try {
      for (const record of _event.Records) {
        ...
        const routeOptions: Array<SQSRoute> = SQSRoutes.getRoutes()
          .filter((route: SQSRoute) => {
            this.logger.trace({ data: route }, 'handle()', 'check condition');
            if (route.condition) {
        ...
        if (!routeOptions || routeOptions.length === 0) {
          this.logger.error('handle()', 'Action is not implemented at this path.');
